...
def _build_excel_row(data, request_id):
    """Build a single data row for the Excel tracker. Returns (row_data, detail_comment)."""
    os_val = data.get('os', '')
    ...
    return row_data, detail_comment


def generate_excel_workbook(data_list, request_ids):
    """Create an Excel workbook with headers and one row per entry.

    Args:
        data_list: list of request data dicts
        request_ids: list of request ID strings (parallel to data_list)

    Returns:
        openpyxl Workbook object (not saved to disk)
    """
    wb = Workbook()
    ws = wb.active
    ...
    for row_num, (data, request_id) in enumerate(zip(data_list, request_ids), start=2):
        row_data, detail_comment = _build_excel_row(data, request_id)
        for col_idx, value in enumerate(row_data, 1):
            ws.cell(row=row_num, column=col_idx, value=value)
        ws.cell(row=row_num, column=23, value=detail_comment)
        ws.cell(row=row_num, column=23).alignment = Alignment(wrap_text=True)
